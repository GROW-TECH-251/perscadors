...
function ProductDetailContent({ product, suggestions, otherResults, searchQuery }: ProductDetailContentProps) {
  ...
  const handleDealWhatsApp = () => {
    ...
    // E11 (réintégration WhatsApp) : photo en URL absolue pour le chat ;
    // la ligne est omise si le produit n'a aucune image (pas de ligne vide).
    const photoUrl = resolvePublicImageUrl(product.image_url || product.images[0]);
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-32 pb-12">
      ...
      <ArticleRequestSection variant="compact" collectionHref={`/categorie/${product.category}`} />
    </div>
  );
}
